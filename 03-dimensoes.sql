...
-- DIM_CATEGORIA
	...
	SELECT DISTINCT
		...
		CASE
			...
			WHEN UPPER(TRIM(`CategoriaProduto`)) LIKE '%RA%'    THEN 'Racao'
		...
		CASE
			WHEN UPPER(TRIM(`CategoriaProduto`)) LIKE '%MED%'    THEN 'Saude e Higiene'
			WHEN UPPER(TRIM(`CategoriaProduto`)) LIKE '%PETISC%' THEN 'Alimentacao'
			WHEN UPPER(TRIM(`CategoriaProduto`)) LIKE '%RA%'     THEN 'Alimentacao'
			WHEN UPPER(TRIM(`CategoriaProduto`)) LIKE '%HIG%'    THEN 'Saude e Higiene'
			WHEN UPPER(TRIM(`CategoriaProduto`)) LIKE '%BRINQ%'  THEN 'Bem-estar'
			WHEN UPPER(TRIM(`CategoriaProduto`)) LIKE '%ACESS%'  THEN 'Bem-estar'
			WHEN UPPER(TRIM(`CategoriaProduto`)) LIKE '%SERV%'   THEN 'Bem-estar'
			ELSE 'Nao Informado'
		END AS grupo_categoria
	FROM stg_pedido
	WHERE TRIM(`CategoriaProduto`) <> '';
...
SELECT
    p.`CodLoja`,
    d.sk_praca,
    CAST(p.`PercentualPublico` AS DECIMAL(6,4))
FROM stg_loja_praca p
JOIN dim_praca d
  ON d.cod_praca = p.`CodPraca`;
