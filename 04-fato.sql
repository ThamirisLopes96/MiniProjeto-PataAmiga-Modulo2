...
SELECT
    ...
    CASE
        WHEN TRIM(p.`DtHoraIntegracaoERP`) = '' THEN NULL
...
LEFT JOIN dim_loja dl
  ON dl.chave_loja =
     CASE
         WHEN REPLACE(REPLACE(UPPER(TRIM(p.`Loja-Nome`)),'/SC',''),'  ',' ') = 'PATA AMIGA BLUMENAL CENTRO'
             THEN 'PATA AMIGA BLUMENAU CENTRO'
         WHEN REPLACE(REPLACE(UPPER(TRIM(p.`Loja-Nome`)),'/SC',''),'  ',' ') = 'PATA AMIGA FLORIPA NORTE'
             THEN 'PATA AMIGA FLORIANOPOLIS NORTE'
         WHEN REPLACE(REPLACE(UPPER(TRIM(p.`Loja-Nome`)),'/SC',''),'  ',' ') = 'PATA AMIGA JGUA DO SUL'
             THEN 'PATA AMIGA JARAGUA DO SUL'
         ELSE REPLACE(REPLACE(UPPER(TRIM(p.`Loja-Nome`)),'/SC',''),'  ',' ')
     END
